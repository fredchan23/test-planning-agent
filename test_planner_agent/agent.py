...
from test_planner_agent.config import MODEL_NAME, PRD_SAMPLE_PATH, DEFAULT_SUT_URL, NVIDIA_API_KEY, NVIDIA_API_BASE
...
intent_mapper = LlmAgent(
    ...
    instruction=(
        ...
        "Analyze the inputs and output a comprehensive Journeys map in Markdown format. Do not use emojis, emoticons, or LaTeX math formatting (e.g., use standard text arrows like '->' instead of $\\rightarrow$)."
    )
)
...
scenario_matrix_agent = LlmAgent(
    ...
    instruction=(
        ...
        "Output the scenario matrix as a structured Markdown table. Do not use emojis, emoticons, or LaTeX math formatting (e.g., use standard text arrows like '->' instead of $\\rightarrow$)."
    )
)
...
element_grounding_agent = LlmAgent(
    ...
    instruction=(
        ...
        "Output this combined data in Markdown format. Do not use emojis, emoticons, or LaTeX math formatting (e.g., use standard text arrows like '->' instead of $\\rightarrow$)."
    )
)
    ...
    for event in ctx.session.events:
        if event.node_info:
            ...
            if event.content and event.content.role == "model":
                text = "".join(part.text for part in event.content.parts if part.text and not getattr(part, "thought", False))
...
"""
    ...
    # Fix LaTeX arrow formatting
    markdown_plan = markdown_plan.replace("$\\rightarrow$", "→").replace("\\rightarrow", "→")
